...
execute store result score #x_deviation esb.pos_xyz run random value -50..50
execute store result score #y_deviation esb.pos_xyz run random value -25..25
execute store result score #z_deviation esb.pos_xyz run random value -50..50
tp @s ^ ^ ^.1
...
execute store result storage esb:esb spruce_needle.y double .009 run scoreboard players operation #y_1 esb.pos_xyz += #y_deviation esb.pos_xyz
